...
| Figure | Subject / Metric | Component | Verification Detail |
| :---: | :--- | :--- | :--- |
| **Fig 1** | GCP Billing December 2025 | Baseline VM Cost | Billed **$10.55** (partial) / **$14.96** forecast |
| **Fig 2** | GCP Cloud Run Service Observability | `alfajar-backend` | Scale-to-zero active instances, low latency |
| **Fig 3** | GCP Billing February 2026 | Cloud Run Savings | **$1.40** forecasted (**-90.87% cost reduction**) |
| **Fig 4** | GCP Billing January 2026 | Baseline Unoptimized VM | Billed **$17.92** total monthly cost |
| **Fig 5** | Cloud Run CLI Deployment Log | `gcloud` CLI | Successful deployment to `us-central1` |
| **Fig 6** | Production VM Docker Status | `ferrari-backend` VM | Running `Up 17 hours (healthy)` on port 8080 |
| **Fig 7** | CI/CD Stabilization & Healthcheck Log | GitHub Actions / Script | 35s evaluation pass & backup container purge |
| **Fig 8** | Auto-Rollback Script Implementation | Shell Logic | Healthcheck `healthy` vs `unhealthy` condition |
